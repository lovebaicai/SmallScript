...
try:
    cur.execute("create table classic(id int PRIMARY KEY AUTO_INCREMENT, title varchar(255), nickname VARCHAR(255),"
...
for tag in sound_tag:
    urltab = 'http://www.ximalaya.com%sclassic' % tag['href']     #urltab是大分类链接
...
#        print urltab2
        ...
        for link in links_title:
            #print link['href']
            args = link['href']
            sql = 'SELECT albumurl FROM allsound WHERE albumurl = (%s)'
            sql2 = 'SELECT albumurl FROM allsound1 WHERE albumurl = (%s)'
            albumnumber = cur.execute(sql, args)
            albumnumber1 = cur.execute(sql2, args)
            if albumnumber ==0 and albumnumber1 == 0:
...
#                    if soundnumber == 0:
                        try:
                            #写入sql
                            cur.execute( "insert into classic(title, nickname, play_count, sound_time, album_title, category_title, coumments_count, favorites_count, soundurl,albumurl)" " values ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')"
