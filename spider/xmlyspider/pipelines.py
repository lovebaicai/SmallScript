...
class XmlyMysqlPipeline(object):
    def __init__(self):
        ...
        try:
            self.cursor.execute(
                "create table paid(id int PRIMARY KEY AUTO_INCREMENT,Albumtitle varchar (255), Albumscore varchar(255),"
    ...
    def process_item(self, item, spider):
        sql = "insert into paid(Albumtitle, Albumscore, TotalPlayCounts, displayDiscountedPrice, Title, Nickname, " \
              "SinglePlayCount, CreatedTime, Duration, LikeCount, CommentsCount, category_title) values " \
              "(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        param = (item['Albumtitle'], item['Albumscore'], item['TotalPlayCounts'], item['displayDiscountedPrice'],
                 item['Title'], item['Nickname'], item['SinglePlayCount'], item['CreatedTime'], item['Duration'],
                 item['LikeCount'], item['CommentsCount'], item['category_title'])
